...
data_page* ReadPageFromFile(uint32_t fid) {
	...
	data_page* ret = (data_page*)pmem_map_file(c, sizeof(data_page), PMEM_FILE_CREATE, 0777, &map_len, &is_pmem);
...
void CreateNewPage(uint32_t fid) {
	...
	string tmp = PM_EHASH_DIRECTORY+to_string((int)fid);
	const char* c = tmp.c_str();
	data_page* tpage = (data_page*)pmem_map_file(c, sizeof(data_page), PMEM_FILE_CREATE, 0777, &map_len, &is_pmem);
	for(int i=0; i<DATA_PAGE_SLOT_NUM; i++) {
		(tpage->slot)[i] = new pm_bucket;
	}
